...
//echo $insert."<pre> gallery <br>"; print_r($data1);
...
//die();
            ...
            if ($insert) {
                ...
                $ses = array("Location Successfully Inserted!");
    ...
    function company_list() {
        ...
        $query = "SELECT * FROM `AdgUserMaster` as co WHERE STATUS = '1'";
        if ($srno != '' || $cmpname != '' || $cntname != '') {

            $query = "SELECT * FROM `AdgUserMaster` WHERE STATUS = '1'";
            if ($cmpname != NULL) {
                $query .= " and UserFName like '%$cmpname%'";
            }
            if ($cntname != NULL) {
                $query .= " and UserMNumber like '%$cntname%'";
            }
            if ($srno != NULL) {
                $query .= " and co.srno like '%$srno%'";
            }

            $data["query"] = $this->master_model->search_data_num($query);
            $totalRows = count($data["query"]);
            $config = array();
            $config["base_url"] = base_url() . "company_master/company_list/search?" . $_SERVER['QUERY_STRING'];
            ...
            $data["query"] = $this->master_model->search_active_record_4($query, $config["per_page"], $page);
            $data['count'] = $page + 1;
        } else {
    ...
    function company_edit() {
        ...
        $data['country_list'] = $this->master_model->get_val("select * from demo_country where status='1' order by name asc");
