...
function set_stock(){
	$this->load->model('daily_reports_model');
	$list = $this->daily_reports_model->get_all('shdailyreadingdetails',array('status'=>'1','location_id'=>'38','date >='=>'2018-07-01'));
	...
	foreach($list as $detail){		
		...
		if($detail->p_opening_original_stock == null){
		...
		if($detail->d_opening_original_stock == ""){
		...
		$Petrol_inventory = $this->daily_reports_model->get_one_data('sh_inventory',array('location_id'=>'38','date'=>$detail->date,'fuel_type'=>'p','status != '=>'0'));
		$Dieasl_inventory = $this->daily_reports_model->get_one_data('sh_inventory',array('location_id'=>'38','date'=>$detail->date,'fuel_type'=>'d','status != '=>'0'));
		$pshort = ($detail->p_total_selling*0.75)/100;
		$dshort = ($detail->d_total_selling*0.75)/100;
