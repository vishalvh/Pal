...
display: none; 
					...
					<br>
					 <div class="cal-md-12">
							...
							<option value="">Select Member </option> 
							 ...
							 <?php if(in_array("petty_cash_report_print",$this->data['user_permission_list'])){ ?>
							   <button class="btn btn-primary" formtarget="_blank"  onClick="return Submitdata();" type="button" >Print</button>
							 <?php } ?>
							 <br>
							 <br></div>
							</form>
						 ...
						 <div class="xs tabls">
							...
							<table class="table">
							  <thead>
								...
								<th>Bank Debit (IN)</th>
								<?php if(in_array("petty_cash_report_action",$this->data['user_permission_list'])){ ?>
								<th>Action</th>
								<?php } ?>
			...
			<div id="sales_on_card_data" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    ...
    <div class="modal-body">
            ...
            <tbody id="sales_on_card_data_add"></tbody>
        </table> 
    </div>
...
$("#sales_on_card_data_add").html("");
        ...
        $.ajax({
				type: "POST",
				url: "<?php echo base_url();?>petty_cash_report/credittotal",
				data: {'location': location,'Employeename':Employeename,"selectdate":selectdate}, // serializes the form's elements.
...
$("#sales_on_card_data_add").html("");
        ...
        $.ajax({
				type: "POST",
				url: "<?php echo base_url();?>petty_cash_report/cashtotal",
				data: {'location': location,'Employeename':Employeename,"selectdate":selectdate}, // serializes the form's elements.
				success: function (data)
				{
					$("#sales_on_card_data_add").html(data);
...
$("#sales_on_card_data_add").html("");
        ...
        $.ajax({
				type: "POST",
				url: "<?php echo base_url();?>petty_cash_report/bankcredittotal",
				data: {'location': location,'Employeename':Employeename,"selectdate":selectdate}, // serializes the form's elements.
				success: function (data)
				{
					$("#sales_on_card_data_add").html(data);
...
$("#sales_on_card_data_add").html("");
        ...
        $.ajax({
				type: "POST",
				url: "<?php echo base_url();?>petty_cash_report/bankdebittotal",
				data: {'location': location,'Employeename':Employeename,"selectdate":selectdate}, // serializes the form's elements.
				success: function (data)
				{
					$("#sales_on_card_data_add").html(data);
...
//		var $tdate = new Date(toDate).toDateString("yyyy-mm-dd");
		...
		if(temp==0){
        document.forms['savingdata'].action='<?php echo site_url()?>petty_cash_report/print_pdf';
...
//		var $tdate = new Date(toDate).toDateString("yyyy-mm-dd");
		...
		if(temp==0){
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>petty_cash_report/reportlist",
