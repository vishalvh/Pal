...
<div class="main-content">
    ...
    <div id="page-wrapper">
        <form method="post" action="<?php echo base_url(); ?>admin/add">			
            <h3 class="blank1" style="margin-top: -20px;">Petty Cash Report</h3>
...
<div class="title-h1">
            <h3 style="text-align:  center;">Credit Report</h3>
        </div>
                 ...
                 <table class="table">
                    ...
                    <tbody>
                     <?php // print_r($list); ?>
<?php if(!empty($cedit_list)){ $i = 1; foreach($cedit_list as $cedit){ 
    if($cedit->type == 'c') { ?>
                           <tr>
                            <td><?php echo  $i; ?> </td>  
                             <td><?php echo  $cedit->name; ?> </td>
                            <td> <?php echo date('d-m-Y', strtotime($cedit->date)) ?></td>
                            <td><?php echo  $cedit->remark; ?> </td>
                            <td><?php echo  $cedit->amount; ?> </td>
                          
                          
                           <?php if($logged_company['type'] == 'c'){ ?>      <td>
						  <a href='<?php echo base_url(); ?>petty_cash_report/info?id=<?php echo $cedit->id; ?>&sdate=<?php echo $this->input->get('sdate'); ?>&edate=<?php echo $this->input->get('edate'); ?>&l_id=<?php echo $this->input->get('l_id'); ?>&member_id=<?php echo $this->input->get('member_id'); ?>' data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-edit"></i></a>       
						  <a  href='<?php echo base_url(); ?>petty_cash_report/delete?id=<?php echo $cedit->id; ?>&sdate=<?php echo $this->input->get('sdate'); ?>&edate=<?php echo $this->input->get('edate'); ?>&l_id=<?php echo $this->input->get('l_id'); ?>&member_id=<?php echo $this->input->get('member_id'); ?>' data-toggle="tooltip" data-original-title="Remove" onclick="return confirm('Are you sure you want to remove this data?');"><i class="fa fa-trash-o"></i></a>
                           </td>  <?php } ?>
                                           </tr>
    <?php $i++; } } }else{ ?>
...
<div class="title-h1">
            <h3 style="text-align:  center;">Debit Report</h3>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                     <?php // print_r($list); ?>
<?php if(!empty($debit_list)){ $i = 1; foreach($debit_list as $cedit){ 
    if($cedit->type == 'd') {?>
                           <tr>
                            <td><?php echo  $i; ?> </td>   
                            <td><?php echo  $cedit->name; ?> </td>
...
<script type="text/javascript">
    function credittotal(selectdate){
alert(selectdate);
    }
    function cashtotal(selectdate){
alert(selectdate);
    }
    function bankcredittotal(selectdate){
alert(selectdate);
    }
    function bankdebittotal(selectdate){
alert(selectdate);
    }
    function printpdf() {
        $(".error").html("");
