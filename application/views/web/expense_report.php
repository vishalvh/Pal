...
display: none; 
			...
			<!-- //header-ends -->
				<div id="page-wrapper">
					<form  >			
								<h3 class="blank1" style="margin-top: -20px;">Expense Report</h3>
					...
					<br>
					 <div class="cal-md-12">
							...
							<select name="location" id="location" class="form-control location" onchange="get_expence(this.value);">
							...
							<div class="col-md-2">
								<select name="exp_name" id="exp_name" class="form-control">
								<option value="">Select Expense</option>
								<?php $cnt = 1; foreach ($list as $raw)  { ?>
  									<option  value="<?php echo $raw['id']; ?>" <?php if($_GET['exp_name'] == $raw['id']){ echo "selected"; } ?>><?php echo $raw['exps_name']; ?></option>
								<?php } ?>
								</select>
								<span class="error" id="exp_name_error"></span>
							</div>
						...
						<!-- 	<div class="col-md-2">
							...
							<?php
							 ...
							 <?php if(in_array("expense_report_print",$this->data['user_permission_list'])){ ?>
						 ...
						 <div class="xs tabls">
							...
							<table class="table">
							  <thead>
								...
								<th>Type</th>
								<th>Reason</th>
                                                          <?php if(in_array("expense_report_action",$this->data['user_permission_list'])){ ?> 	<th>Action</th> <?php } ?>
...
function get_expence(id){
	$('#exp_name').html('<option value="">Select Expense</option>');
	if(id!=""){
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>expense/get_exp_typ",
			data: {'lid': id}, // serializes the form's elements.
			success: function (data)
			{
				$("#exp_name").append(data);
			}
		});
	}else{
...
//		var $tdate = new Date(toDate).toDateString("yyyy-mm-dd");
		...
		if(temp==0){
        document.forms['savingdata'].action='<?php echo site_url()?>expense/print_pdf';
...
//		var $tdate = new Date(toDate).toDateString("yyyy-mm-dd");
		...
		if(temp==0){
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>expense/reportlist",
				data: {'lid': lid,'sdate':sdate,'edate':edate,"exp_name":exp_name}, // serializes the form's elements.
				success: function (data)
				{
					//alert($data);
					$("#newdata").html(data);
