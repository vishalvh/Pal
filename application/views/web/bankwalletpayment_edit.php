			...
			<!-- //header-ends -->
				<div id="page-wrapper">
					<form method="post" action="<?php echo base_url();?>admin/add">			
								<h3 class="blank1" style="margin-top: -20px;">Wallet Payment Edit</h3>
					...
					<?php if ($this->session->flashdata('success')) { ?>
						...
						?>
						 <div class="md tabls">
						 <form class="form-horizontal" method="post" action="<?php echo base_url();?>bank_deposit/wallet_payment_edit?id=<?php echo $bd[0]->id; ?>&sdate=<?php echo $this->input->get('sdate'); ?>&edate=<?php echo $this->input->get('edate'); ?>&l_id=<?php echo $this->input->get('l_id'); ?>" name="savingdata" onsubmit="return validate()">
							<div class="bs-example4" data-example-id="contextual-table" style="display:  inline-block;width: 100%;">
								...
								<input  id="amount" type="text" class="form-control1 "  placeholder="Card Amount" name="card_amount" value="<?php echo $bd[0]->card_amount;?>">
                                   	...
                                   	<div class="col-md-12" id="extra_div_id" style="margin-top: 15px;">
								<div class="col-md-2" >
									<label class="control-label"><b>Wallet Type</b></label>
									</div>
								<div class="col-sm-4">
								<select id="extra_id" class="form-control1" name="extra_id" onchange="change_batch()">
								<option value="">Select Payment Type</option>
								<?php foreach($extras as $extra){ ?>
								<option value="<?php echo $extra->id; ?>" <?php if($bd[0]->extra_id == $extra->id ){ echo "selected"; };?> ><?php echo $extra->name; ?></option>
								<?php } ?>
								</select>
								<?php echo form_error('extra_id','<div class="error" style="color:red;padding-bottom: 0px;padding-top: 0px;">','</div>' );?>
									<div class="invalid-feedback" id="extra_iderror" style="color: red;"></div>
...
<a href="#" onclick="window.history.back();">Back</a>
								...
								<input type="hidden" name="batch" id="batch" value="<?php echo $bd[0]->batch_no;?>">
								</form>
						   </div>
						</div>
				<!-- switches -->
...
function change_batch(){
	var batch = $("#extra_id option:selected").text();
	$("#batch").val(batch);
}
