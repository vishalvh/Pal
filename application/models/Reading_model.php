<?php class Reading_model extends CI_Model {
...
WHERE STATUS = 1 ORDER BY id ASC ");     
	...
	function shdailyreadingdetails_reading_list_details_id($id){
        $query = $this->db->query("SELECT srh.id as pid, sp.`id`,sp.`name`, srh.`Reading`,srh.`PumpId`,sp.`type` FROM `ShDailyReadingDetails` AS drd LEFT JOIN `shadminmaster` ON shadminmaster.`id` = drd.`UserId` LEFT JOIN shreadinghistory AS srh ON srh.`RDRId` = drd.`id` LEFT JOIN shpump AS sp ON sp.`id` = srh.`PumpId` WHERE drd.`status`='1' AND drd.id = '$id' ORDER BY drd.id DESC ");     
        return $query->result_array();
    }
		function shdailyreadingdetails_reading_list_details2($id){
        $query = $this->db->query("SELECT srh.id as pid, sp.`id`,sp.`name`, srh.`Reading`,sp.`type` FROM `ShDailyReadingDetails` AS drd LEFT JOIN `shadminmaster` ON shadminmaster.`id` = drd.`UserId` LEFT JOIN shreadinghistory AS srh ON srh.`RDRId` = drd.`id` LEFT JOIN shpump AS sp ON sp.`id` = srh.`PumpId` WHERE drd.`status`='1' AND drd.id = '$id' ORDER BY drd.id DESC ");     
        return $query->result_array();
    }
