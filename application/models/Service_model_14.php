<?php
class Service_model_14 extends CI_Model {
...
/////////////////////////////////////////////////vishal 11-4 strat
	...
	function get_pump_list_oil($type, $location) {
        $query = $this->db->query("SELECT id,name,type,p_qty as packet_value,packet_value as p_price,p_type as packet_type,packet_type as description,stock FROM shpump WHERE status='1' and type='$type' and location_id='$location' ORDER BY p_type ASC");
        return $query->result_array();
    }
...
////////////////////////////////////////////////vishal 11-4 end
    ...
    function get_user_list($id = "", $lid = "") {
        ...
        $query = $this->db->query("SELECT * FROM sh_userdetail WHERE status='1'AND `show` = 1 and block = '1' $cnd ORDER BY name ASC");
        return $query->result_array();
    }
...
WHERE  scd.status = '1'" . $q . $q1 . $q2 . $q3);
    ...
    public function company_creditors_list($company_id = "", $location_id = "") {
        ...
        $query .= "AND sh_creditors.status = 1 and sh_location.status = 1 ORDER BY sh_creditors.`name` ASC ";
        $q = $this->db->query($query);
        return $q->result();
    }
...
AND c.`bankdeposit_id`  = $id");
    ...
    public function get_expence_list($id) {
        $query = $this->db->query("SELECT * FROM sh_expensein_types  WHERE status = '1' and location_id = '$id' order by exps_name asc");
        return $query->result();
    }
