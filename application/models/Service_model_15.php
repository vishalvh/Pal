<?php
class Service_model_15 extends CI_Model {
...
//        $this->db->where_not_in('id', $id);
	...
	public function master_fun_get_tbl_val_new_new($dtatabase, $condition, $order) {
...
//        $this->db->where_not_in('id', $id);
        ...
        // $data['user'] = $query->result_array();
        // return $data['user'];
		$li = $condition['location_id'];
		$query = $this->db->query("SELECT * FROM `shpump` WHERE `location_id` = '$li' AND `status` = 1 AND `type` = 'O' order by shpump.order asc");
        return $query->result();
    }

    public function master_fun_get_tbl_val_select($select, $dtatabase, $condition, $order) {
    ...
    public function master_fun_get_tbl_val_company_employee($dtatabase, $condition, $order, $location_id = "") {
        ...
        $this->db->where('shusermaster.Active', 1);
    ...
    SUM(si.`o_amount`+si.oil_cgst+si.oil_sgst) AS ostock,
    SUM(si.d_quantity) AS dstock,
    SUM(si.`p_quantity`) AS pstock,
...
FROM
    ...
    WHERE shusermaster.Active = 1 and shusermaster.company_id = '$company_id'
...
GROUP BY si.`date` ");
    ...
    public function master_fun_get_tbl_val_company_dailyprice($date_to = "", $date_from = "", $current_date = "", $location_id = "") {


        if ($location_id != "") {
            $q = " AND sd.user_id = '$location_id'";
        } else {
            $q = "";
        }
        if ($date_from != "") {
...
FROM
  sh_dailyprice AS sd 
WHERE sd.`status`='1'" . $q . $q1 . $q2 . " ORDER BY DATE");
    ...
    public function master_fun_get_tbl_val_company_reading($date_to = "", $date_from = "", $location_id = "") {
  ...
  shusermaster.UserFName,
  sh_location.l_name,
  shdailyreadingdetails.id,
  shdailyreadingdetails.date,
  shdailyreadingdetails.created_at,
  shdailyreadingdetails.oil_reading,
  shdailyreadingdetails.p_tank_reading,
  shdailyreadingdetails.d_tank_reading,
  shdailyreadingdetails.p_deep_reading,
  shdailyreadingdetails.d_deep_reading,
  shdailyreadingdetails.p_total_selling,
  shdailyreadingdetails.d_total_selling,
  shdailyreadingdetails.p_testing,
  shdailyreadingdetails.d_testing,
  shdailyreadingdetails.cash_on_hand
...
FROM
  ...
  JOIN sh_location
    ON sh_location.l_id = shusermaster.l_id 
WHERE shdailyreadingdetails.status = '1' and shusermaster.Active =1  " . $q1 . $q2 . $q3 . "order by date ");
...
WHERE drd.status='1' ";
    ...
    function expense_details($location_id, $date) {
        $query = $this->db->query("SELECT she.`reson` ,ex.`exps_name`,she.`amount` FROM sh_expensein_details AS she JOIN sh_expensein_types AS ex ON ex.`id` = she.`expense_id` WHERE she.`location_id` = '$location_id' and she.date = '$date'");
        return $query->result_array();
    }
...
/////////////////////////////////////////////////vishal 11-4 strat
    function get_pump_list($type, $location) {
        $query = $this->db->query("SELECT id,name,type,p_qty as packet_value,packet_value as p_price,p_type as packet_type,packet_type as description,xp_type FROM shpump WHERE status='1' and type='$type' and location_id='$location' and shpump.show = '1' ORDER BY name ASC");
        return $query->result_array();
    }
	function get_pump_list_oil($type, $location) {
        $query = $this->db->query("SELECT id,name,type,p_qty as packet_value,packet_value as p_price,p_type as packet_type,packet_type as description,stock FROM shpump WHERE status='1' and type='$type' and location_id='$location' ORDER BY shpump.order ASC");
        return $query->result_array();
    }
	
	function get_pump_list_oil_new($location,$date) {
        $query = $this->db->query("SELECT 
  shpump.id,
  shpump.name,
  shpump.type,
  shpump.p_qty AS packet_value,
  sh_oil_daily_price.`sel_price` AS p_price,
  shpump.p_type AS packet_type,
  shpump.packet_type AS description,
  sh_oil_daily_price.stock 
FROM
  shpump 
  JOIN 
  sh_oil_daily_price ON sh_oil_daily_price.`o_p_id` = shpump.id AND sh_oil_daily_price.`date` = '$date'
WHERE shpump.status = '1' 
  AND shpump.type = 'o' 
  AND shpump.location_id = '$location' 
ORDER BY shpump.order ASC ");
        return $query->result_array();
    }
...
////////////////////////////////////////////////vishal 11-4 end
    ...
    function get_user_list($id = "", $lid = "") {
        ...
        $query = $this->db->query("SELECT * FROM sh_userdetail WHERE status='1'AND `show` = 1 and block = '1' and active_status = 'Active' $cnd ORDER BY name ASC");
        return $query->result_array();
    }
    ...
    public function master_fun_get_tbl_val_company_expense($user_id = "", $date_to = "", $date_from = "", $current_date = "", $company_id = "", $location_id) {
        ...
        $query = $this->db->query("SELECT
               ...
               sh_expensein_details  
                ...
                AND sh_expensein_details.status = '1' shusermaster.Active = 1  " . $q . $q1 . $q2 . $q3);
...
JOIN shexpensein ON shexpensein.id = sh_expensein_types.exps_id WHERE sh_expensein_details.id='$ex_id'  ");
    ...
    public function master_fun_get_tbl_val_company_bankdeposit($date_to = "", $date_from = "", $current_date = "", $location_id) {
...
FROM
  ...
  JOIN sh_location AS shl 
    ON shl.l_id = sh.l_id 
WHERE  so.status = '1' and  sh.Active = 1 " . $q . $q1 . $q2 . $q3);
...
WHERE  scd.status = '1'" . $q . $q1 . $q2 . $q3);
    ...
    public function company_worker_list($company_id = "", $location_id = "") {
        ...
        $query .= "AND sh_workers.status = 1 AND sh_workers.show = 1 ORDER BY sh_workers.`name` ASC ";
    ...
    public function emp_worker_list($company_id = "") {
        $query = $this->db->query("SELECT 
            ...
            AND sh_workers.status = 1 AND sh_workers.show = 1 ORDER BY  sh_workers.`name` ASC ");
        return $query->result();
    }
    ...
    public function company_creditors_list($company_id = "", $location_id = "") {
        ...
        $query .= "AND sh_creditors.status = 1 and sh_creditors.show = 1 and sh_location.status = 1 ORDER BY sh_creditors.`name` ASC ";
        $q = $this->db->query($query);
        return $q->result();
    }
    ...
    public function get_worker_loan_detail($worker_id, $date) {
        $query = $this->db->query("SELECT IFNULL(sum(loan_amount),0) as loan_total,IFNULL(SUM(paid_loan_amount),0) paid_loan_amount  FROM sh_workers_salary WHERE  worker_id='$worker_id' and status = '1'");
        return $query->row();
    }
...
WHERE  sws.status ='1' AND sws.location_id='$company_id' AND  MONTH(sws.date) = '$mont' AND YEAR(sws.date) ='$year' AND sws.payment_type ='$type' ");
    ...
    public function last_month_data($mont,$year,$l_id) {
        $query = $this->db->query("SELECT budget  FROM sh_last_day_entry  WHERE   MONTH(sh_last_day_entry.date) = '$mont' AND YEAR(sh_last_day_entry.date) ='$year' and sh_last_day_entry.`location_id` = $l_id");
        return $query->row();
    }
    ...
    function get_oil_pckg_list($id, $location_id = "") {
		...
		$this->db->order_by("shr.p_type", 'asc');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    function expence_date($locationid, $edate, $sdate) {
         $query = $this->db->query("SELECT DATE,SUM(amount) total FROM `sh_expensein_details` WHERE DATE <= '$edate' AND DATE >= '$sdate' AND location_id = '$locationid' AND STATUS='1' GROUP BY DATE ORDER BY DATE ASC");
         return $query->result();

        // $this->db->select('shi.amount AS total ,location.l_name,user.UserFName,shi.id,shi.date As DATE,shi.reson,typ.exps_name');
        ...
        // }
        // if ($locationid != "") {
            // $this->db->where('user.l_id', $locationid);
        // }
        // if ($sdate == "" && $edate == "") {
    ...
    function inventory_detail_call($date, $locationid, $type) {
        $query = $this->db->query("SELECT * from sh_inventory where status = '1' and date = '$date' and location_id = '$locationid' and fuel_type ='$type'");
        return $query->row();
    }
...
and location_id = '$id' AND batch_no IS NOT NULL GROUP BY DATE");
    ...
    function get_tankreading($tank) {
        $sql = "SELECT * FROM `sh_tank_dep_reding` WHERE `status` = 1 AND `tanktype` = '$tank' order by cm asc";
        $query = $this->db->query($sql);
        return $query->result();
    }

    function get_pump_list_Detail($id) {
        $sql = "SELECT p.name,p.packet_value,p.spacket_value,p.p_qty,p.packet_type,p.p_type,dh.* FROM shreadinghistory dh JOIN shpump p ON p.id=dh.PumpId WHERE dh.status = '1' AND dh.RDRId = '$id' ";
        $query = $this->db->query($sql);
        return $query->result();
    }
    public function meter_details($id,$date){
		$query = $this->db->query("SELECT 
    srh.`created_at`,su.UserFName,sp.id,sp.`name`,sp.`type`,sp.`packet_type`,sp.`packet_value`,sp.`p_qty`,sp.`p_type`,srh.`qty`,srh.`Reading`, d_price.`bay_price`,d_price.`sel_price`
  FROM
   ...
   LEFT  JOIN  sh_oil_daily_price AS d_price
     ON d_price.`o_p_id` = sp.`id` AND d_price.`date` = srh.`date`
	 join shusermaster as su on su.id = srh.UserId
  WHERE srh.`RDRId` = '$id' 
    AND dr.status = '1'
	AND srh.status = '1'
	AND sp.status = '1'
  ...
  ORDER BY sp.type,sp.name ASC");
    ...
    public function online_list_view($date, $lid) {
        $this->db->select('id,sh_onlinetransaction.amount,sh_onlinetransaction.cheque_tras_no,sh_onlinetransaction.date, 2 as flage');
        $this->db->from('sh_onlinetransaction');
        $this->db->where('status', '1');
        $this->db->where('date = ', $date);
        $this->db->where('location_id', $lid);
...
and location_id = '$id' AND batch_no IS NOT NULL ");
    ...
    public function view_d_amount($id) {
        $query = $this->db->query("SELECT u.`name`, l.`l_name`,c.* FROM sh_credit_debit AS c
JOIN  `sh_location` AS l ON l.`l_id` = c.`location_id` 
JOIN `sh_userdetail` AS u ON u.`id` = c.`customer_id`
WHERE c.`status` = 1
AND l.`status` = 1
AND c.`transaction_type` != 'cs'
AND c.`bankdeposit_id`  = $id");
        return $query->result();
    }

    public function compcustliast($id) {
        $query = $this->db->query("SELECT `sh_userdetail`.`id`, `sh_userdetail`.`name`, `shl`.`l_id`, `shl`.`l_name`, `sh_userdetail`.`phone_no`,`sh_userdetail`.`address`, `sh_userdetail`.`cheque_no`, `sh_userdetail`.`bank_name`, `sh_userdetail`.`personal_guarantor`,  `sh_userdetail`.`status` FROM `sh_userdetail` INNER JOIN `sh_location` as `shl` ON `shl`.`l_id` = `sh_userdetail`.`location_id` WHERE `shl`.`l_id` = '$id' AND `sh_userdetail`.`status` = 1 order by `sh_userdetail`.`name` asc");
        return $query->result();
    }

    public function get_expence_list($id) {
        $query = $this->db->query("SELECT * FROM sh_expensein_types  WHERE status = '1' and location_id = '$id'AND  sh_expensein_types.show ='1' order by exps_name asc");
        return $query->result();
    }
	public function master_fun_get_tbl_val_with_order($dtatabase, $condition, $order) {
		$this->db->order_by($order[0],$order[1]);
        $query = $this->db->get_where($dtatabase, $condition);
        $data['user'] = $query->result();
        return $data['user'];
    }
	public function master_fun_get_tbl_val_new($dtatabase, $condition, $order,$select) {
		$this->db->select($select);
        $query = $this->db->get_where($dtatabase, $condition);
        $data['user'] = $query->result_array();
        return $data['user'];
    }
	public function get_user_entry_onlinetransaction($uid,$date){
		$query = $this->db->query("SELECT so.`paid_by`,so.`id`,so.`amount`,so.`bank_name`,so.`cheque_tras_no`,so.`invoice_no`,sc.`name` FROM `sh_onlinetransaction` so JOIN sh_creditors sc ON sc.id = so.`customer_name` WHERE so.`user_id` = '$uid' AND so.`status` = 1 AND so.`date` = '$date'");
        return $query->result();
	}
	public function bankdeposit_credi_debit($id){
		$query = $this->db->query("SELECT * form sh_expensein_details WHERE `user_id` = '$id' and status = '1'");
        return $query->result();
	}
	public function date_credi_debit($date,$id){
		$query = $this->db->query("SELECT scd.id,scd.`payment_type`,scd.`amount`,scd.`bill_no`,scd.`fuel_type`,scd.`remark`,scd.`vehicle_no`,scd.`quantity`,su.`name` FROM `sh_credit_debit` scd 
JOIN `sh_userdetail` su ON su.`id` = scd.`customer_id`
WHERE scd.`user_id` = '$id' and scd.date = '$date' and scd.status ='1' ");
        return $query->result();
	}
        public function get_credit_debit_data($date,$l_id){
		$query = $this->db->query("SELECT * FROM `sh_credit_debit` 
WHERE DATE = '$date'
AND location_id = '$l_id'
AND payment_type != 'c'");
        return $query->result();
	}
	public function get_data_expense($id,$date){
		$this->db->select('ed.id,ed.user_id,ed.date,ed.status,ed.all_data,ed.amount,ed.created_at,ed.reson,et.exps_name');
		$this->db->from('sh_expensein_details as ed');
		$this->db->join('sh_expensein_types as et','et.id = ed.expense_id','left');
		$this->db->where('ed.user_id',$id);
		$this->db->where('ed.date',$date);
    ...
    public function oil_view_details($id){
		$query = $this->db->query("SELECT sh_oil_inventory_data.*,p.p_type,p.`name`,p.`location_id`,p.`packet_type`,p.`packet_type`,p.`spacket_value`,p.`stock` FROM `sh_oil_inventory_data` 
JOIN `shpump` AS p ON p.`id` = sh_oil_inventory_data.`p_id` WHERE o_m_id = '$id' ");
        return $query->result();
	}
        function get_oil_detail_separate_price($user_id,$date){
		$sql ="SELECT  p.`name`,p.`p_type`,o.`stock`, p.`spacket_value`,r_history.`Reading`, o.`date` FROM sh_oil_daily_price AS o JOIN shpump AS p ON p.id = o.`o_p_id` JOIN `shreadinghistory` AS r_history ON r_history.`PumpId` = p.`id` AND r_history.`date`= o.`date` WHERE r_history.`UserId` = '$user_id' AND o.date = '$date'  ";
		$query = $this->db->query($sql);
		return $query->result();
	}
        function update_profit($month,$year){
		$sql ="SELECT 
  MONTH(DATE) AS month,
  YEAR(DATE) AS year,
  location_id , 
  SUM(p_total_selling) AS p_selling,
  SUM(d_total_selling) AS d_selling 
FROM
  `shdailyreadingdetails` 
  where MONTH(DATE) = $month AND YEAR(DATE) = $year
GROUP BY MONTH(DATE),location_id ";
		$query = $this->db->query($sql);
		return $query->result();
	}
         public function get_bankdeposit_entry($l_id, $date,$id){
		$query = $this->db->query("SELECT * FROM sh_bankdeposit  WHERE DATE = '$date' AND location_id = '$l_id' and user_id = '$id'");
        return $query->result();
	}
         public function get_credit_debit_data_by_user($l_id, $date,$id){
		$query = $this->db->query("SELECT 
  sh_credit_debit.*,sh_userdetail.`name` AS customer_name
FROM
  sh_credit_debit 
  JOIN sh_bankdeposit 
    ON sh_bankdeposit.`id` = sh_credit_debit.`bankdeposit_id` 
   JOIN sh_userdetail 
   ON sh_userdetail.`id` = sh_credit_debit.`customer_id`      WHERE sh_credit_debit.date = '$date' AND sh_credit_debit.location_id = '$l_id' and sh_credit_debit.payment_type = 'd' and sh_credit_debit.`user_id` = '$id'");
        return $query->result();
	}
    public function tankInventoryList($lid,$sdate,$edate) {
        $this->db->select('*');
        $this->db->from('tank_inventory');
        $this->db->where('status', '1');
        $this->db->where('date <= ', $edate);
        $this->db->where('date >=', $sdate);
        $this->db->where('location_id', $lid);
	...
	public function tankInventorydetail($id) {
        $this->db->select('*');
        $this->db->from('tank_inventory_list');
        $this->db->where('status', '1');
        $this->db->where('tank_inventory_id', $id);
        $this->db->order_by('id', 'ASC');
        $q1 = $this->db->get();
        return $q1->result();
    }
	public function meterInventorydetail($id) {
        $this->db->select('*');
        $this->db->from('tank_inventory_meter');
        $this->db->where('status', '1');
        $this->db->where('tank_inventory_id', $id);
        $this->db->order_by('id', 'ASC');
        $q1 = $this->db->get();
	...
	public function getOilInventoryList($date) {
        $this->db->select('*');
        $this->db->from('sh_oil_inventory_data_master');
        $this->db->where('status', '1');
        $this->db->where('date >= ', $date);
        $q1 = $this->db->get();
        return $q1->result();
    }
	public function oilListArray($id) {
        $this->db->select('*');
        $this->db->from('shpump');
        $this->db->where('status', '1');
		...
		foreach($list as $detail){
			$finalList[$detail->id] = $detail;
		}
		return $finalList;
    }
	public function oilData($id) {
        $this->db->select('*');
        $this->db->from('sh_oil_inventory_data');
        //$this->db->where('status', '1');
        $this->db->where('o_m_id', $id);
        $q1 = $this->db->get();
        return $q1->result();
    }
	public function oilAddedInv($lid,$date) {
        $this->db->select('*');
        $this->db->from('sh_inventory');
        $this->db->where('status', '1');
        $this->db->where('fuel_type', 'o');
        $this->db->where('location_id', $lid);
        $this->db->where('date', $date);
        $q1 = $this->db->get();
        return $q1->row();
