<?php class Service_model12 extends CI_Model {
...
WHERE sh.company_id = '$company_id' 
  AND scd.status = '1'".$q.$q1.$q2.$q3); 
	...
	public function inventory_detail_for_add($locationid,$date,$fule){
		$query = $this->db->query("SELECT oil_total_amount,o_stock,d_stock,p_stock,p_new_price as p_price,d_new_price as d_price,d_quantity,p_quantity FROM `sh_inventory` WHERE `location_id` = '$locationid' AND `date` = '$date' And fuel_type ='$fule' AND `status` = '1'"); 
      return $query->row();
	}
