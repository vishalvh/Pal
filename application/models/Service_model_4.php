<?php class Service_model_4 extends CI_Model {
...
WHERE  sws.status ='1' AND sws.location_id='$company_id' AND  MONTH(sws.date) = '$mont' AND YEAR(sws.date) ='$year' AND sws.payment_type ='$type' "); 
    ...
    {
      $this->db->select('id,sh_bankdeposit.date,deposit_amount,withdraw_amount,amount as depositamonut, cheque_no,bank_name, 1 as flage');
...
and location_id = '$id' AND batch_no IS NOT NULL ");       
	...
	public function compcustliast($id){		
		$query = $this->db->query("SELECT `sh_userdetail`.`id`, `sh_userdetail`.`name`, `shl`.`l_id`, `shl`.`l_name`, `sh_userdetail`.`phone_no`, `sh_userdetail`.`cheque_no`, `sh_userdetail`.`bank_name`, `sh_userdetail`.`personal_guarantor`, `sh_userdetail`.`address`, `sh_userdetail`.`status` FROM `sh_userdetail` INNER JOIN `sh_location` as `shl` ON `shl`.`l_id` = `sh_userdetail`.`location_id` WHERE `shl`.`l_id` = '$id' AND `sh_userdetail`.`company_id` = '1' AND `sh_userdetail`.`status` = 1 order by `sh_userdetail`.`name` asc");       
		return $query->result();	
	}
}
?>