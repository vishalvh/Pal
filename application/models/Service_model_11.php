<?php
class Service_model_11 extends CI_Model {
...
FROM
  ...
  JOIN sh_location
    ON sh_location.l_id = shusermaster.l_id 
WHERE shdailyreadingdetails.status = '1' " . $q1 . $q2 . $q3 . "order by date ");
...
/////////////////////////////////////////////////vishal 11-4 strat
	...
	function get_pump_list_oil($type, $location) {
        $query = $this->db->query("SELECT id,name,type,p_qty as packet_value,packet_value as p_price,p_type as packet_type,packet_type as description FROM shpump WHERE status='1' and type='$type' and location_id='$location' ORDER BY p_type ASC");
        return $query->result_array();
    }
...
and location_id = '$id' AND batch_no IS NOT NULL GROUP BY DATE");
    ...
    function get_pump_list_Detail($id) {
        $sql = "SELECT p.name,p.packet_value,p.p_qty,p.packet_type,p.p_type,dh.* FROM shreadinghistory dh JOIN shpump p ON p.id=dh.PumpId WHERE dh.status = '1' AND dh.RDRId = '$id' ";
        $query = $this->db->query($sql);
        return $query->result();
    }
