<?php
class Service_model_13 extends CI_Model {
...
////////////////////////////////////////////////vishal 11-4 end
    ...
    function get_user_list($id = "", $lid = "") {
        ...
        $query = $this->db->query("SELECT * FROM sh_userdetail WHERE status='1' and block = '1' $cnd ORDER BY name ASC");
        return $query->result_array();
    }
...
and location_id = '$id' AND batch_no IS NOT NULL GROUP BY DATE");
    ...
    function get_pump_list_Detail($id) {
        $sql = "SELECT p.name,p.packet_value,p.spacket_value,.p_qty,p.packet_type,p.p_type,dh.* FROM shreadinghistory dh JOIN shpump p ON p.id=dh.PumpId WHERE dh.status = '1' AND dh.RDRId = '$id' ";
        $query = $this->db->query($sql);
        return $query->result();
    }
...
AND c.`bankdeposit_id`  = $id");
	...
	public function date_credi_debit($date,$id){
		$query = $this->db->query("SELECT scd.id,scd.`payment_type`,scd.`amount`,scd.`transaction_type`,scd.`transaction_number`,scd.`bank_name`,su.`name` FROM `sh_credit_debit` scd 
...
WHERE scd.`user_id` = '$id' and scd.date = '$date' and scd.status ='1' ");
    ...
    public function oil_view_details($id){
		$query = $this->db->query("SELECT sh_oil_inventory_data.*,p.`name`,p.`location_id`,p.`packet_type`,p.`packet_type`,p.`spacket_value`,p.`stock` FROM `sh_oil_inventory_data` 
