<?php class Service_model_1 extends CI_Model {
...
public function get_one($dtatabase, $condition) {
  ...
  function oilparday($oil,$date){
    $query = $this->db->query("SELECT * FROM sh_oil_daily_price where o_p_id ='$oil' and date='$date'");   

    return $query->row();
  }
  function selloilparday($oil,$date){
	  $date = date('Y-m-d', strtotime($date . ' +1 day'));
    $query = $this->db->query("SELECT * FROM shreadinghistory where PumpId ='$oil' and date='$date' and status ='1'");   
...
JOIN AdgUserMaster ON AdgUserMaster.id = demo_query.user_id where demo_query.status ='1'");   
  ...
  function sh_oil_inventory_data_master(){
    $query = $this->db->query("SELECT * FROM sh_oil_inventory_data_master");   

    return $query->result();
  }
  function alloil($id){
    //$query = $this->db->query("SELECT * FROM shpump where type='O' and status=1 and company_id ='25' and ( id='339' or id='279'  or id='201' or id='200')");
	//$query = $this->db->query("SELECT * FROM shpump where type='O' and status=1 and company_id ='25' and ( id='200' )");
$query = $this->db->query("SELECT * FROM shpump where type='O' and status=1 and company_id ='25' and location_id ='".$id."'");
    return $query->result();
  }
  
  function sh_oil_inventory_data(){
    $query = $this->db->query("SELECT * FROM  `sh_oil_inventory_data` WHERE sh_oil_inventory_data.`newprice` IS NOT NULL ORDER BY sh_oil_inventory_data.date ASC");   

    return $query->result();
  }
  
  function alldata($ins){
	  $text = '"invoice_no":"';
    $query = $this->db->query("SELECT * FROM alldata where data like '%".$text.$ins."%' and url LIKE '%oil_inventory%' ");   
...
WHERE  so.status = '1'".$q.$q1.$q2.$q3); 
  ...
  $query = $this->db->query("SELECT 
    scd.`amount`,scd.`date`,scd.`payment_type`,scd.`customer_id`,scd.`bill_no`,scd.`vehicle_no`,scd.`quantity`,scd.`remark`,scd.`transaction_number`,scd.`bank_name`,scd.`batch_no`,scd.`fuel_type`
FROM
...
WHERE  scd.status = '1'".$q.$q1.$q2.$q3); 
	...
	public function get_worker_loan_detail($worker_id,$date){
		$query = $this->db->query("SELECT IFNULL(sum(loan_amount),0) as loan_total,IFNULL(SUM(paid_loan_amount),0) paid_loan_amount  FROM sh_personal_loan WHERE  worker_id='$worker_id' and date <= '$date'"); 
      return $query->row();
	}
