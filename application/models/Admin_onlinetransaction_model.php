<?php
class Admin_onlinetransaction_model extends CI_Model
{
  ...
  private function _get_datatables_query()
    ...
    $this->db->select('shc.name,location.l_name,user.UserFName,shi.id,shi.date,shi.invoice_no,shi.customer_name,shi.amount,shi.paid_by,shi.cheque_tras_no');  
...
WHERE sh.company_id='$cid' and sh.status='1'");
    ...
    public function exportCSV($fdate = "",$tdate = "",$company="",$location="",$employeename=""){
        $response = array();
        $this->db->select('DATE_FORMAT(shi.date,"%d/%m/%Y") as date,shc.name,location.l_name,user.UserFName,shi.invoice_no,shi.customer_name,shi.amount,shi.paid_by,shi.cheque_tras_no');  
        $this->db->join('shusermaster user','user.id=shi.user_id');
        $this->db->join('sh_location location','location.l_id=user.l_id');
        $this->db->join('sh_com_registration as shc','shc.id=user.company_id');
        $this->db->from('sh_onlinetransaction as shi');
