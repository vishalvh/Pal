<?php
class Admin_creditdebit_model extends CI_Model
{
    var $table = 'sh_credit_debit as shi';
    var $column_order = array(null, 'location.l_name','user.UserFName','ud.name','shi.date','shi.payment_type','shi.bill_no','shi.vehicle_no','shi.fuel_type','shi.amount','shi.quantity'); //set column field database for datatable orderable
   var $column_search = array('location.l_name','user.UserFName','ud.name','shi.date','shi.payment_type','shi.bill_no','shi.vehicle_no','shi.fuel_type','shi.amount','shi.quantity'); //set column field database for datatable searchable
  ...
  private function _get_datatables_query()
    ...
    $this->db->select('shc.name as company_name,location.l_name,user.UserFName,shi.id,ud.name,shi.date,shi.payment_type,shi.bill_no,shi.vehicle_no,shi.fuel_type,shi.amount,shi.quantity');   
    $this->db->join('shusermaster user','user.id=shi.user_id');
    $this->db->join('sh_location location','location.l_id=user.l_id');
    $this->db->join('sh_userdetail ud','ud.id=shi.customer_id');
   ...
   public function count_all($extra="")
    ...
    public function creditdebit_info($id)
    {
      $this->db->select('*');
      $this->db->from('sh_credit_debit as shi');
...
WHERE sh.company_id='$cid' and sh.status='1'");
    ...
    public function exportCSV($fdate = "",$tdate = "",$company="",$location="",$employeename=""){
        $response = array();
         $this->db->select('DATE_FORMAT(shi.date,"%d/%m/%Y") as date,shc.name as company_name,user.UserFName,location.l_name,ud.name,shi.payment_type,shi.bill_no,shi.vehicle_no,shi.fuel_type,shi.amount,shi.quantity');   
          $this->db->join('shusermaster user','user.id=shi.user_id');
          $this->db->join('sh_location location','location.l_id=user.l_id');
          $this->db->join('sh_userdetail ud','ud.id=shi.customer_id');
          $this->db->join('sh_com_registration as shc','shc.id=user.company_id');
