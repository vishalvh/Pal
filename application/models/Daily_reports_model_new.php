<?php class Daily_reports_model_new extends CI_Model {
	...
	public function getPastLoadOfWorker($worker_id,$date){
		$sql = "SELECT (IFNULL(SUM(loan_amount),0) - IFNULL(SUM(paid_loan_amount),0)) as loan FROM sh_workers_salary WHERE STATUS = '1' AND date < '$date' AND worker_id = '$worker_id'";
	...
	public function worker_salary_remening($cid,$month,$year,$worker_id = ""){
			...
			$lasymonthldastdate = date("Y-n-j", strtotime($month." last day of previous month"));
		...
		IFNULL( (SELECT IFNULL(SUM(loan_amount),0) - IFNULL(SUM(paid_loan_amount),0) FROM sh_workers_salary WHERE STATUS = '1' AND date <= '$lasymonthldastdate' AND worker_id = sh_workers.id), 0 ) AS past_loan_amount,
		IFNULL( (SELECT  SUM(loan_amount) FROM sh_workers_salary  WHERE STATUS = '1' AND DATE >= '$month'  AND DATE <= '$year' AND worker_id = sh_workers.id), 0 ) AS advance,
                 IFNULL( (SELECT  salary FROM sh_workers_monthly_salary  WHERE  month = '$mo'  AND year = '$y' AND workers_id = sh_workers.id), 0 ) AS salary   
FROM sh_workers 
				...
				$query2 .= " order by sh_workers.code asc";
...
AND sts.`location_id`='$location_id' AND sts.`date` >= '$sdate' AND sts.`date` <= '$edate'";
	...
	function get_tank_reading_sales($id){
		$sql ="SELECT stwr.`date`,stwr.`deepreading`,stwr.`tank_id`,stwr.`id`,stwr.`sales_id`,stwr.`volume`,stl.`fuel_type`,stl.`tank_name`,stl.`id`,stl.`xp_type` FROM `sh_tank_wies_reading_sales` stwr 
...
ORDER BY shpump.id ASC");
	...
	public function update_oilstock($date,$key,$oildiffrent){
		//echo "UPDATE sh_oil_daily_price SET `stock` = stock+".$oildiffrent." WHERE `o_p_id` ='".$key."' AND DATE >= '".$date."'";
        $sql = "UPDATE sh_oil_daily_price SET `stock` = stock+".$oildiffrent." WHERE `o_p_id` ='".$key."' AND DATE >= '".$date."'";
        $this->db->query($sql);
		return 1;
    }
	}