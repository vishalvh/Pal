<?php
class Inwardreport_model extends CI_Model
{
...
WHERE sh.company_id='$cid' and sh.status='1'");
    ...
    public function exportCSV($fdate = "",$tdate = "",$company="",$location="",$employeename=""){
        $response = array();
        $this->db->select('DATE_FORMAT(shi.date,"%d/%m/%Y") as date,shc.name,user.UserFName,location.l_name,shi.pi_number,shi.p_fuelamount,shi.pv_taxamount,shi.p_paymenttype,shi.p_chequenumber,shi.p_paidamount,shi.p_quantity,shi.p_tankerreading,shi.di_number,shi.d_fuelamount,shi.dv_taxamount,shi.d_paymenttype,shi.d_chequenumber,shi.d_paidamount,shi.d_quantity,shi.d_tankerreading,shi.oil_type,shi.o_quantity,shi.oil_amount');
        $this->db->from('sh_inventory as shi');
        $this->db->join('shusermaster user','user.id=shi.user_id');
        $this->db->join('sh_location location','location.l_id=user.l_id');
        $this->db->join('sh_com_registration as shc','shc.id=user.company_id');
