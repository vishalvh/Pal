<?php class Service_model_5 extends CI_Model {
...
FROM
  ...
  JOIN sh_location
    ON sh_location.l_id = shusermaster.l_id 
WHERE shdailyreadingdetails.status = '1' ".$q1.$q2.$q3); 
...
AND c.`bankdeposit_id`  = $id");       
	...
	public function compcustliast($id){		
		$query = $this->db->query("SELECT `sh_userdetail`.`id`, `sh_userdetail`.`name`, `shl`.`l_id`, `shl`.`l_name`, `sh_userdetail`.`phone_no`, `sh_userdetail`.`cheque_no`, `sh_userdetail`.`bank_name`, `sh_userdetail`.`personal_guarantor`,  `sh_userdetail`.`status` FROM `sh_userdetail` INNER JOIN `sh_location` as `shl` ON `shl`.`l_id` = `sh_userdetail`.`location_id` WHERE `shl`.`l_id` = '$id' AND `sh_userdetail`.`status` = 1 order by `sh_userdetail`.`name` asc");       
		return $query->result();	
	}
