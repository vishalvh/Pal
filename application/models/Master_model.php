...
WHERE drd.`status`='1' AND drd.created_at = '$date' ORDER BY drd.id DESC LIMIT $two,$one ");
        return $query->result_array();
    }
...
//		    $this->db->join('`shpump` sp ', 'sp.id = rh.PumpId', 'left');
	...
	public function check_email1_add($email){ 
        $sql = "SELECT AdminEmail FROM shadminmaster WHERE AdminEmail='$email' AND status ='1' and type = 2";
        $result = $this->db->query($sql)->result_array();
        return $result;
    }
