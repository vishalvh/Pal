...
class Company_master extends CI_Controller {
    ...
    function check_database1($email) {
        $return_value = $this->master_model->check_email1_add($email);
        if ($return_value) {
            $this->form_validation->set_message('check_database1', 'Sorry, This Email is already used by another user please Use another one');
            return FALSE;
        } else {
            return TRUE;
        }
...
//print_r($_POST);
        ...
        $this->form_validation->set_rules('Adminemail', 'Email', 'trim|required|callback_check_database1');
        $this->form_validation->set_rules('Adminpassword', 'Password', 'trim|required');
        $this->form_validation->set_rules('AdminMnumber', 'Mobile Number', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
           ...
           // $this->load->view('company_view', $data);
            ...
            $AdminMnumber = $this->input->post("AdminMnumber");
...
//echo "<pre> company <br>"; print_r($data1);
            ...
            $data["user"] = $this->user_model->getUser($data["logged_in"]["id"]);
            if ($insert) {
                $ses = array("Employee Successfully Inserted!");
...
//            $data["user"] = $this->user_model->getUser($data["logged_in"]["id"]);
        ...
        $data["category"] = $this->master_model->company_list();
       // print_r($data); die();
        $this->load->view('Admin/header');
        $this->load->view('Admin/nav', $data);
    ...
    function company_delete() {
        ...
        $delete = $this->master_model->master_fun_update1("shadminmaster", 'id', $this->uri->segment('3'), $data);
        if ($delete) {
            $ses = array("Customer Successfully Deleted!");
            $this->session->set_userdata('success', $ses);
...
//		print_r("<pre>");print_r($data);
        ...
        $data['query'] = $this->master_model->master_get_tbl_val_id("shadminmaster", array("id" => $this->uri->segment('3')), array("id", "desc"));
		//print_r($data['query']);
        $this->form_validation->set_rules('AdminEmail', 'Email', 'trim|required');
        $this->form_validation->set_rules('AdminGender', 'Gender', 'trim|required');
        $this->form_validation->set_rules('AdminMNumber', 'Mobile Number', 'trim|required');
      //  $this->form_validation->set_rules('AdminMNumber', 'Mobile', 'trim|required');
        if ($this->form_validation->run() != FALSE) {
			//print_r($_POST);
            $AdminEmail = $this->input->post("AdminEmail");
            $AdminGender = $this->input->post("AdminGender");
            $AdminMNumber = $this->input->post("AdminMNumber");
            $AdminName = $this->input->post("AdminName");
           ...
           // $change_pass = $this->input->post("change_pass");
			...
			//print_r($_POST); die();
if($change_pass == '1' ){
	//$this->form_validation->set_rules('AdminPassword', 'Password', 'required|trim|callback_check_data');
            $data1 = array(
                "AdminName" => $AdminName,
                "AdminEmail" => $AdminEmail,
	...
	$data2 = array(
		"AdminName" => $AdminName,
            ...
            $update = $this->master_model->master_fun_update1("shadminmaster", 'id', $this->uri->segment('3'), $data1);
			$this->session->set_userdata('logged_in', $data2);
}else{
	$data1 = array(
                "AdminName" => $AdminName,
                "AdminEmail" => $AdminEmail,
	...
	$data2 = array(
		"AdminName" => $AdminName,
            ...
            if ($update) {
                $ses = "Customer Successfully Updated!";
...
//		print_r("<pre>");print_r($data);
	...
	//print_r($data1); die();
            ...
            if ($update) {
              $ses = array("Customer Successfully Inserted!");
            $this->session->set_userdata('success', $ses);
            redirect('company_master/company_list', 'refresh');
            }
        } else {
