...
//              $this->email->to($email.',jabir@virtualheight.com');
    ...
    public function login() {
        ...
        if($email != NULL && $password != NULL) {
            ...
            if ($result != "") {
                $data = $this->service_model->master_fun_get_tbl_val("shusermaster", array("UserEmail" => $email, "UserPassword" => $password, "status" => 1), array("id", "asc"));
                $uniqueId = substr(base_convert(sha1(uniqid(mt_rand())), 16, 36), 0, 8);
                $data1 = array("token" => $uniqueId);
                $Active = $data[0]['Active'];
                $id = $data[0]['id'];
                $update = $this->service_model->master_fun_updateid("shusermaster", $id, $data1); 
                if ($Active == '1') {
                    $row = $this->service_model->master_num_rows("shusermaster", array("id" => $data[0]['id'], "status" => 1));
                    $membercount = $this->service_model->master_num_rows("shusermaster", array("id" => $data[0]['id'], "status" => 1));
                    $data[0]['team_count'] = "$row";
                    $list = array();
                    $list = $this->service_model->master_fun_get_tbl_val("shusermaster", array("id" => $data[0]['id'], "status" => 1), array("id", "id"));
                    $user_team = array();
                    $user_team = $this->service_model->select_all("shusermaster",$id);
                    $petrol = $this->service_model->get_pump_list('P',$data[0]['l_id']);
                    $diesel = $this->service_model->get_pump_list('D',$data[0]['l_id']);
                    $oil = $this->service_model->get_pump_list('O',$data[0]['l_id']);
                    $expensein = $this->service_model->expensein_type_list();
                    $fina_Array = array();
                    foreach($expensein as $expenseindetail){
                        $expenseindetail['expensein_detail'] = $this->service_model->selectbyidarray('sh_expensein_types',$data[0]['company_id'],'comapny_id');
                    ...
                    $data2 = $this->service_model->master_fun_get_tbl_val_service("shusermaster", array("UserEmail" => $email, "UserPassword" => $password, "status" => 1), array("id", "asc"));
                ...
                if ($result != "") {
                $data = $this->service_model->master_fun_get_tbl_val("sh_com_registration", array("email" => $email, "password" => $password, "status" => 1), array("id", "asc"));
                $uniqueId = substr(base_convert(sha1(uniqid(mt_rand())), 16, 36), 0, 8);
                $data1 = array("token" => $uniqueId);
                $Active = $data[0]['active'];
                $id = $data[0]['id'];
                $update = $this->service_model->master_fun_updateid("sh_com_registration", $id, $data1); 
                if ($Active == '1') {
                    $row = $this->service_model->master_num_rows("sh_com_registration", array("id" => $data[0]['id'], "status" => 1));
                    $membercount = $this->service_model->master_num_rows("sh_com_registration", array("id" => $data[0]['id'], "status" => 1));
                    $data[0]['team_count'] = "$row";
                    $list = array();
                    $list = $this->service_model->master_fun_get_tbl_val("sh_com_registration", array("id" => $data[0]['id'], "status" => 1), array("id", "id"));
                    // $user_team = array();
                    // $user_team = $this->service_model->select_all("shusermaster",$id);
                    ...
                    $fina_Array = array();
                    foreach($expensein as $expenseindetail){
                        $expenseindetail['expensein_detail'] = $this->service_model->selectbyidarray('sh_expensein_types',$expenseindetail['id'],'exps_id');
                        array_push($fina_Array,$expenseindetail);
                    }
                    $data[0]['token'] = $uniqueId;
                    $meter_reading['petrol'] = $petrol;
                    ...
                    $data2 = $this->service_model->master_fun_get_tbl_val_service_1("sh_com_registration", array("email" => $email, "password" => $password, "status" => 1), array("id", "asc"));
                    echo $this->json_data("1", "",array($data[0]));
                } else {
                    echo $this->json_data("0", "Please Verify Your Email Address", "");
                }
	...
	$company = "";
	if($token != "" ){
        $tokendata = $this->service_model->master_fun_get_tbl_val_3("shusermaster", array("token" => $token, "status" => 1), array("id", "asc"));
        ...
        $petrol = $this->service_model->get_pump_list('P',$location);
        //$data['petrol'] = $petrol;
        $diesel = $this->service_model->get_pump_list('D',$location);
        //$data['diesel'] = $diesel;
        //$oil = $this->service_model->oil_type_list();
		$oil = $this->service_model->get_pump_list('O',$location);
        $expensein = $this->service_model->expensein_type_list();
        $fina_Array = array();
        foreach($expensein as $expenseindetail){
            
            $expenseindetail['expensein_detail'] = $this->service_model->selectbyidarray('sh_expensein_types',$company,'comapny_id');
		...
		$data['gst'] = 9;
		$data['oil_benift'] = 5;
		$data['vesion_code'] = $finalarray = array("vesion_code"=>"14","flag"=>"TRUE");
    ...
    public function forget_password() {
        ...
        if ($email != NULL) {
            ...
            $row = $this->service_model->master_fun_get_tbl_val_1("shusermaster", array("UserEmail" => $email, "status" => 1), array("id", "asc"));
        ...
        $user = $row[0]['UserFName'];
                ...
                $insert = $this->service_model->master_fun_update_UserEmail1('shusermaster',$email,$data1);
...
//          print_r($update); die();
    ...
    public function employee_detail() {
        $token = $this->input->get_post('token');
        if ($token != "") {
              $data = $this->service_model->master_fun_get_tbl_val_3("shusermaster", array("token" => $token, "status" => 1), array("id", "asc"));
             if ($data) {
                $data = $this->service_model->employee_detail("shusermaster", array("status" => 1), array("id", "asc"));
                echo $this->json_data("1", "", $data);
            } else {
                echo $this->json_data("3", "Token expired. Please login again.", "");
            }
    ...
    public function edit_profile() {
        ...
        if ($token != "" && $name != "" && $email != "" && $mobile != "") {
            $list = $this->service_model->master_fun_get_tbl_val("shusermaster", array("token" => $token, "status" => 1), array("id", "id"));
            //print_r($list);
            if($list){
        ...
        $email2 =  $user[0]['UserEmail'];
            ...
            if ($email2 == $email) {
                ...
                $update = $this->service_model->master_fun_update("shusermaster", $token, $data);
                if ($update) {
                    echo $this->json_data("1", "Your Profile has been Updated","" );
                }}
                else {
                $row = $this->service_model->master_num_rows("shusermaster", array("UserEmail" => $email, "status" => 1));
                if ($row == 0) {
                    $data = array(
                    "UserFName" => $name,
                    "UserEmail" => $email,
...
function addoil($location_id,$date){
					...
					$inventoryoil = $this->service_model->get_inventory_detail('O',$location_id,$date);
					if($inventoryoil){}else{
						
						$newdate = date('Y-m-d', strtotime('-1 day', strtotime($date)));
						$oildata = $this->service_model->inventory_detail_for_add($location_id,$newdate,"o");
						
						if($oildata){
							$oil_total_amount = $oil_total_amount+$oildata->oil_total_amount;
							$salesdata = $this->service_model->sels_detail_for_add($location_id,$newdate);
							if($salesdata){
						...
						$data = array(
							 ...
							 $this->service_model->master_insert("sh_inventory",$data);
					}
}
    public function add_details() {
        ...
        if ($token != "" ) {
            ...
            if($list){
				...
				$checkdata = $this->service_model->master_fun_get_tbl_val("shdailyreadingdetails", array("UserId" => $id,"location_id"=>$location_id,"date"=>$date,"status" => 1), array("id", "asc"));
				if($checkdata){
...
$this->addoil($location_id,$date);
    ...
    public function dailyprice_add() {
        ...
        if($token != "" && $date != "" && $pet_price != "" && $dis_price != "")
        {
            ...
            if($list)
            {
                ...
                $id = $list[0]['l_id']; 
				$checkdata = $this->service_model->master_fun_get_tbl_val("sh_dailyprice", array("user_id"=>$id,"date"=>$date,"status" => 1), array("id", "asc"));
    ...
    public function add_inventory()    {
        ...
        if($token != "" && $date != "")
        {
            ...
            if ($list)
            {
				$checkentry = $this->service_model->master_fun_get_tbl_val("sh_inventory", array("date" => $date,"fuel_type"=>$fuel_type,"status" => 1,"location_id"=>$list['0']['l_id']), array("id", "asc"));
				
				if($checkentry){
					echo $this->json_data("0","Sorry you already added detail.","");
				}else{
					$newdate = date('Y-m-d', strtotime('-1 day', strtotime($date)));
					$oildata = $this->service_model->inventory_detail_for_add($list['0']['l_id'],$newdate,"o");
					
					if($oildata){
						$oil_total_amount = $oil_total_amount+$oildata->oil_total_amount;
						$salesdata = $this->service_model->sels_detail_for_add($list['0']['l_id'],$newdate);
						if($salesdata){
							$oil_total_amount = $oil_total_amount-$salesdata->oil_reading;
						}
					}
...
'deep_reading'=>$deep_reading								
					...
					foreach($pamoutnew as $row){
						...
						$this->service_model->master_insert("sh_oil_inventory",$data1);
    ...
    public function change_password() {
        ...
        if ($token != "" && $password != "" && $oldpassword != "") {
            ...
            $data2 = $this->service_model->master_fun_get_tbl_val_3("shusermaster", array("token" => $token, "status" => 1), array("id", "asc"));
            if($data2){

            $row = $this->service_model->master_fun_get_tbl_val("shusermaster", array("token" => $token, "status" => 1), array("id", "asc"));
  ...
  $prefs = array( 'format' => 'sql', // gzip, zip, txt 
    ...
    public function user_list(){
        ...
        if ($token != "") 
        {
                ...
                if ($list) 
                {
                    $data = $this->service_model->get_user_list($list[0]['company_id'],$list[0]['l_id']);
					if($data){
                    echo $this->json_data("1", "", $data);
					}else{
            ...
            if ($list) 
            {
                if ($payment_type == 'd' && $customer_id == '0') 
                {
                    ...
                    if ($customer_name != "")
                    {
                        ...
                        $insert = $this->service_model->master_insert("sh_credit_debit",$data);
    ...
    public function company_registration()    {
        ...
        {
                $row = $this->service_model->master_num_rows("sh_com_registration", array("email" => $email, "status" => 1));
                if ($row == 0) 
                {
                    date_default_timezone_set('Asia/Kolkata');
                    $data =  array();
                    ...
                    $insert = $this->service_model->master_insert("sh_com_registration",$data);
                    echo $this->json_data("1","Your Data submitted successfully","");
                }
                else
                {
    ...
    public function company_editprofile()    {
        ...
        if ($token != "" && $name != "" && $email != "" && $mobile != "") 
        {
            $list = $this->service_model->master_fun_get_tbl_val("sh_com_registration", array("token" => $token, "status" => 1), array("id", "id"));
            if ($list) 
            {
                $user = $this->service_model->companylist_active_record($token);
                if ($user) 
                {
                    $email2 =  $user[0]['email'];
                    if ($email2 == $email) 
                    {
                        ...
                        $data = array(
                            ...
                            $update = $this->service_model->master_fun_update("sh_com_registration", $token, $data);
                            echo $this->json_data("1", "Your Profile has been Updated","" );
                    }
                    else
                    {
    ...
    public function company_location()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                   $data = $this->service_model->master_fun_get_tbl_val_company_location("sh_location", array("company_id" =>$company_id ,"status" => 1), array("id", "asc"));
if($data){
    ...
    public function company_pump(){
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                ...
                $data = $this->service_model->master_fun_get_tbl_val_company_pump("shpump", $condition, array("id", "asc"));
                echo $this->json_data("1","",$data);
            }
            else
            {
    ...
    public function company_employee()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                   $data = $this->service_model->master_fun_get_tbl_val_company_employee("shusermaster", array("shusermaster.company_id" =>$company_id ,"shusermaster.status" => 1,"shusermaster.location_id"), array("id", "asc"),$location_id);
				   if($data){
                   echo $this->json_data("1","",$data);
        ...
        if ($token != "" && $location_name != "" && $phone_no != "" && $address != "" && $tank_type != "" && $tin != "" && $dealar != "" && $gst != "" && !empty($_FILES['logo']['name'])) 
        {
            ...
            if ($list) 
            {
                ...
                date_default_timezone_set('Asia/Kolkata');
                    ...
                    $insert = $this->service_model->master_insert("sh_location",$data);
                    echo $this->json_data("1","Your Data submitted successfully","");
    ...
    public function add_employee()    {
        ...
        if ($token != "" && $name != "" && $mobile != "" && $email != "" && $password != "" && $location_id != "" && $shift != "") 
        {
            ...
            if ($list) 
            {
                $email1 = $this->service_model->master_fun_get_tbl_val("shusermaster", array("UserEmail" => $email, "status" => 1), array("id", "id"));
                ...
                else
                {
                    ...
                    $insert = $this->service_model->master_insert("shusermaster",$data);
                    echo $this->json_data("1","Your Data submitted successfully","");
                }
        ...
        if ($token != "" && $name != "" && $type != "" && $location_id != "") 
        {
            ...
            if ($list) 
            {
                ...
                date_default_timezone_set('Asia/Kolkata');
                    ...
                    $insert = $this->service_model->master_insert("shpump",$data);
                    echo $this->json_data("1","Your Data submitted successfully","");
            }
            else
            {
        ...
        if ($token != "" && $location_name != "" && $location_id != "" && $phone_no != "" && $address != "" && $tank_type != "" && $tin != "" && $dealar != "" && $gst != "" ) 
        {
            ...
            if ($list) 
            {
                ...
                date_default_timezone_set('Asia/Kolkata');
                    ...
                    $this->service_model->master_fun_update_location("sh_location",$location_id,$data);
    ...
    public function oil_package_list(){
        ...
        if($token){
            $list = $this->service_model->master_fun_get_tbl_val("sh_com_registration", array("token" => $token, "status" => 1), array("id", "id"));
            if($list){
                $olist = $this->service_model->get_oil_pckg_list($list[0]['id'],$location_id);
    ...
    public function add_oil_package(){
        ...
        if($token != "" && $location_id != "" && $pump_name != "" && $packet_value != "" && $packet_type != "" && $p_type != "" && $p_qty != ""){
            ...
            if($list){
                ...
                $oil_package_insert_id = $this->service_model->add_oil_pckg($data);
    ...
    public function edit_oil_package(){
        ...
        if($token != "" && $location_id != "" && $pump_name != "" && $packet_value != "" && $packet_type != "" && $p_type != "" && $p_qty != "" && $pkg_id != ""){
            ...
            if($list){
                ...
                $response = $this->service_model->update_oil_pckg($pkg_id,$data);
    ...
    public function edit_employee()    {
        ...
        if ($id != "" && $token != "" && $name != "" && $mobile != "" && $email != "" && $password != "" && $location_id != "" && $shift != "") 
        {
            ...
            if ($list) 
            {
                $email1 = $this->service_model->master_fun_get_tbl_val("shusermaster", array("UserEmail " => $email,"id !="=>$id, "status" => 1), array("id", "id"));
                ...
                else
                {
                    ...
                    $this->service_model->master_fun_update_employee("shusermaster",$id,$data);
                    echo $this->json_data("1","Your Data Updated successfully","");    
                }
        ...
        if ($token != "" && $name != "" && $type != "" && $location_id != "" && $id != "") 
        {
            ...
            if ($list) 
            {
                ...
                date_default_timezone_set('Asia/Kolkata');
                    ...
                    $this->service_model->master_fun_update_employee("shpump",$id,$data);
                    echo $this->json_data("1","Your Data updated successfully","");
            }
            else
            {
    ...
    public function inventory_report(){
        ...
        {      
            ...
            if ($list) 
            {
                ...
                $data = $this->service_model->master_fun_get_tbl_val_company_inventory_1($user_id,$date_to,$date_from,$current_date,$list[0]['id'],$location_id);
                if($data == null){
    ...
    public function dailyprice_report()    {
        ...
        if ($token != "") 
        {
            ...
            {
                $company_id = $list[0]['id'];
                $data = $this->service_model->master_fun_get_tbl_val_company_dailyprice($user_id,$date_to,$date_from,$current_date,$company_id,$location_id);
                if($data){
                echo $this->json_data("1","",$data);
                }else{
                    echo $this->json_data("0","Data not available",$data);
    ...
    public function reading_report()    {
        ...
        if ($token != "") 
        {
            ...
            if ($list) 
            {
                $data = $this->service_model->master_fun_get_tbl_val_company_reading($user_id,$date_to,$date_from,$company_id,$location_id);
                if($data){
                    echo $this->json_data("1","",$data);
                }else{
    ...
    public function company_change_password() {
        ...
        if ($token != "" && $password != "" && $oldpassword != "") {
            ...
            $data2 = $this->service_model->master_fun_get_tbl_val_company("sh_com_registration", array("token" => $token, "status" => 1), array("id", "asc"));
            if($data2){

            $row = $this->service_model->master_fun_get_tbl_val("sh_com_registration", array("token" => $token, "status" => 1), array("id", "asc"));
    ...
    public function company_forget_password() {
        ...
        if ($email != NULL) {
            ...
            $row = $this->service_model->master_fun_get_tbl_val_1("sh_com_registration", array("email" => $email, "status" => 1), array("id", "asc"));
            $title = $this->config->item('title');
            if ($row != null) {
                $uniqueId=  substr(base_convert(sha1(uniqid(mt_rand())), 16, 36), 0, 5);
                ...
                send_mail($email,$title.' forgotten Password',$message);
                $data1 = array("Rs" => $uniqueId);
                $insert = $this->service_model->master_fun_update_companyEmail('sh_com_registration',$email,$data1);
                echo $this->json_data("1", "An email has been sent to your email address", "");
            } else {
    ...
    public function company_customer_detail()    {
        ...
        if ($token != "") 
        {
             ...
             {
                $company_id = $list[0]['id'];
                $data = $this->service_model->master_fun_get_tbl_val_company_customer("sh_userdetail", array("sh_userdetail.company_id" =>$company_id ,"sh_userdetail.status" => 1), array("id", "asc"),$location_id);
				if($data){
    ...
    public function employee_user_detail()    {
        ...
        if ($token != "") 
        {
             ...
             if ($list) 
             {
                ...
                $data = $this->service_model->master_fun_get_tbl_val_company_customer("sh_userdetail", array("sh_userdetail.company_id" =>$company_id ,"sh_userdetail.status" => 1), array("id", "asc"));
                echo $this->json_data("1","",$data);
        ...
        if ($token != "" && $customer_name != "" && $phone_no != "" && $address != "" && $cheque_no !="" && $bank_name != "" && $personal_guarantor != "" && $location_id != "") 
        {
            ...
            if ($list) 
            {
                $company_id = $list[0]['id'];
                    ...
                    $insert = $this->service_model->master_insert("sh_userdetail",$data);
                    echo $this->json_data("1","Your Data submitted successfully","");
            }
            else
            {
        ...
        if ($token != "" && $customer_name != "" && $id != ""  && $phone_no != "" && $address != "" && $cheque_no !="" && $bank_name != "" && $personal_guarantor != "" && $location_id != "") 
        {
            ...
            if ($list) 
            {
                $company_id = $list[0]['id'];
                    ...
                    $this->service_model->master_fun_update_customer("sh_userdetail",$id,$data);
                    echo $this->json_data("1","Your Data Updated successfully","");
            }
            else
            {
    ...
    public function expense_report()    {
        ...
        if ($token != "" ) 
        {
                ...
                {
                        $company_id = $list[0]['id'];
                        $data = $this->service_model->master_fun_get_tbl_val_company_expense($user_id,$date_to,$date_from,$current_date,$company_id,$location_id);
                        if($data){
                        echo $this->json_data("1","",$data);
    ...
    public function expence_detail()    {
        ...
        if ($token != "" && $ex_id != "") 
        {
                ...
                if ($list) 
                {
                        ...
                        $data = $this->service_model->master_fun_get_tbl_val_expense_detail($company_id,$ex_id);
    ...
    public function bankdeposit()    {
        ...
        if($token != "" )
        {
            ...
            if ($list) 
            {
                ...
                $data = $this->service_model->master_fun_get_tbl_val_company_bankdeposit($user_id,$date_to,$date_from,$current_date,$company_id,$location_id);
    ...
    public function onlinetransaction()    {
        ...
        if($token != "" )
        {
            ...
            if ($list) 
            {
                ...
                $data = $this->service_model->master_fun_get_tbl_val_company_onlinetrans($user_id,$date_to,$date_from,$current_date,$company_id,$location_id);
               if($data){
                echo $this->json_data("1","",$data);
			   }else{
				   echo $this->json_data("0","Data no available.","");
    ...
    public function creditdebit()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                ...
                $data = $this->service_model->master_fun_get_tbl_val_company_credit_debit($user_id,$date_to,$date_from,$current_date,$company_id,$location_id);
                 if($data){
    ...
    public function company_user_detail()    {
        ...
        if ($token != "") 
        {
            ...
            if ($list) 
            {
                ...
                $data = $this->service_model->company_user_detail($company_id);
        ...
        if ($token != "" && $name != "" && $shift != "" && $location_id != "" && $mobile != "" && $address != "" && $salary !="") 
        {
            ...
            if ($list) 
            {
                $company_id = $list[0]['id'];
                    ...
                    $insert = $this->service_model->master_insert("sh_workers",$data);
        ...
        if ($token != "" && $name != "" && $shift != "" && $location_id != "" && $mobile != "" && $address != "" && $salary !="") 
        {
            ...
            if ($list) 
            {
				...
				$data = array(
                    ...
                    $this->service_model->master_fun_update_customer("sh_workers",$id,$data);
                    echo $this->json_data("1","Your Data Updated successfully","");
            }
            else
            {
    ...
    public function worker_list()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                   $data = $this->service_model->company_worker_list($company_id,$location_id);
        ...
        if ($token != "" && $name != "" && $location_id != "" ) 
        {
            ...
            if ($list) 
            {
                $company_id = $list[0]['id'];
                    ...
                    $insert = $this->service_model->master_insert("sh_creditors",$data);
		...
		if ($token != "" && $name != "" && $location_id != "" ) 
        {
            ...
            if ($list) 
            {
				...
				$data = array(
                    ...
                    $this->service_model->master_fun_update_customer("sh_creditors",$id,$data);
                    echo $this->json_data("1","Your Data Updated successfully","");
            }
            else
            {
    ...
    public function creditors_list()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                    $data = $this->service_model->company_creditors_list($company_id,$location_id);
				   if($data){
    ...
    public function company_creditors_list()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                   $data = $this->service_model->company_creditors_list($company_id);
	...
	public function add_salary(){
		...
		if ($token != "" && $worker_id != "" && $amount != ""  && $date != "" ) 
        {
            ...
            if ($list) 
            {
                    ...
                    $insert = $this->service_model->master_insert("sh_workers_salary",$data);
	...
	public function worker_remaning_salsry(){
		...
		if ($token != "" && $worker_id != "" ) 
        {
            ...
            if ($list) 
            {
				$month = date("m");
				$year = date("Y");
				   $data = $this->service_model->worker_salary_remening($worker_id,$month,$year);
	...
	public function emply_worker_list()    {
        ...
        if($token != "")
        {
            ...
            if ($list) 
            {
                   $data = $this->service_model->emp_worker_list($location_id);
				   if($data){
						echo $this->json_data("1","",$data);
				   }else{
					   echo $this->json_data("0","Worker not available",$data);
    ...
    public function tank_dep_reding(){
		$list = $this->service_model->master_fun_get_tbl_val("sh_tank_dep_reding", array("status" => 1), array("id", "asc"));
	...
	public function daily_price(){
		...
		if($token != "" && $date != "" )
        {
            $list = $this->service_model->master_fun_get_tbl_val("shusermaster", array("token" => $token, "status" => 1), array("id", "asc"));
            if($list)
            {
				$companyid = $list[0]['l_id'];
				$data = $this->service_model->master_fun_get_tbl_val("sh_dailyprice", array("user_id"=>$companyid,"date"=>$date,"status" => 1), array("id", "asc"));
				if($data){
					echo $this->json_data("1","",$data);
			   }else{
    ...
    public function meter_reading_report(){
		...
		if($token != "" && $date != "" && $location_id != "")
        {
            ...
            {
				$companyid = $list[0]['id'];
				$data = $this->service_model->master_fun_get_tbl_val_meter_reading_data($date,$companyid,$location_id);
				if($data){
					echo $this->json_data("1","",$data);
                }else{
	...
	public function daily_price_new(){
		...
		if($token != "" && $date != "" )
        {
            ...
            if($list)
            {
				...
				$data = $this->service_model->master_fun_get_tbl_val("sh_dailyprice", array("user_id"=>$location,"date"=>$date,"status" => 1), array("id", "asc"));
				
				if($data){
					
					$inventorypetrol = $this->service_model->get_inventory_detail('P',$location,$date);
					if($inventorypetrol){
						$inventorydiesel = $this->service_model->get_inventory_detail('D',$location,$date);
						if($inventorydiesel){
							// $inventoryoil = $this->service_model->get_inventory_detail('O',$location,$date);
							// if($inventoryoil){}else{
					...
					foreach ($petrol as $plist){
						$get_preding = $this->service_model->master_fun_get_tbl_val("shreadinghistory", array("PumpId"=>$plist['id'],"date"=>$newdate,"status" => 1), array("id", "asc"));
						if($get_preding){
	...
	public function inventory_detail_for_add(){
		...
		if($token != "" && $date != ""){
            ...
            if ($list){
				$location_id = $list[0]['l_id'];
				$petroldata = $this->service_model->inventory_detail_for_add($location_id,$date,"p");
				$dieseldata = $this->service_model->inventory_detail_for_add($location_id,$date,"d");
				$oildata = $this->service_model->inventory_detail_for_add($location_id,$date,"o");
//echo $this->db->last_Query();
				$salesdata = $this->service_model->sels_detail_for_add($location_id,$date);
	...
	public function detail_for_last_mnth(){
		...
		if($token != ""){
            ...
            if ($list){
            	$companyid = $list[0]['company_id'];
            	$location_id = $list[0]['l_id'];
            	$totalsalary = $this->service_model->total_salary($companyid);
				$totalpaid = $this->service_model->total_paid_salary($companyid,date('m',strtotime($date)),date('Y',strtotime($date)));
				$totaldebit = $this->service_model->total_credit($location_id,date('m',strtotime($date)),date('Y',strtotime($date)),'d');
				$TotalCredit = $this->service_model->total_credit($location_id,date('m',strtotime($date)),date('Y',strtotime($date)),'c');
				$total_onlinetransaction = $this->service_model->total_onlinetransaction($location_id,date('m',strtotime($date)),date('Y',strtotime($date)));
				$total_bankdeposit = $this->service_model->total_bankdeposit($location_id,date('m',strtotime($date)),date('Y',strtotime($date)));
		...
		if($token != ""){
            ...
            if ($list){
            	...
            	$data = array(
            		...
            		$insert = $this->service_model->master_insert("sh_last_day_entry",$data);
	...
	public function add_loan(){
		...
		if($token != "" && $date != "" )
        {
			...
			if ($list) {
				$id = $list[0]['id'];
				$location_id = $list[0]['l_id']; 
				$data = array("worker_id"=>$worker_id,"user_id"=>$id,"location_id"=>$location_id,"loan_amount"=>$amount,'remark'=>$remark);
				$insert = $this->service_model->master_insert("sh_personal_loan",$data);
				if($insert){
					echo $this->json_data("1","Your Data submitted successfully","");             		
				}else{
					echo $this->json_data("0","Some think is wrong","");
