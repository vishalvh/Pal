...
<div class="main-content">
    ...
    <div id="page-wrapper">
        <form method="post" action="<?php echo base_url(); ?>admin/add">			
            <h3 class="blank1" style="margin-top: -20px;">Oil stock</h3>
        </form>
        <h3 class="blank1"></h3>
            ...
            <div class="bs-example4" data-example-id="contextual-table">
                ...
                <div class="over-scroll js-scroll bdr">
                    <table class="table">
							...
							<td>Name</td>
							<td>Stock</td>
							<?php for($i=date('Y-m-d', strtotime($sdate. ' +1 day')); strtotime($edate) >= strtotime($i);$i = date('Y-m-d', strtotime($i . ' +1 day'))){ ?>
							<td nowrap><?php echo date('d-m-Y',strtotime($i)); ?></td>
							<?php } ?>
							<td>Stock</td>
							</thead>
							<tbody>
								<?php foreach ($startoilstock as $oil_data){ ?>
								
								<tr>
									<td nowrap><?php echo $oil_data->packet_type ?></td>
									<td><?php echo $oil_data->stock; ?></td>
									<?php for($i=date('Y-m-d', strtotime($sdate. ' +1 day')); strtotime($edate) >= strtotime($i);$i = date('Y-m-d', strtotime($i . ' +1 day'))){ ?>
									<td data-toggle="tooltip" title=" <?php echo $oil_data->packet_type ?> <?=date('d/m/Y', strtotime($i))?>" ><?php if(isset($oilsellingparday[$i][$oil_data->id])) { echo $oilsellingparday[$i][$oil_data->id]; }else{ echo '0'; } ?></td>
									<?php } ?>
									<td >
