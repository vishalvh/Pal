...
class Reading_master extends CI_Controller {
...
//echo "<pre> company <br>"; print_r($data1);
    ...
    function reading_list() {
		...
		$date1 = "";
		$date2 = "";
	$date1new =$this->input->get("date1");
		if($date1new != ""){
			$date1 = date("Y-m-d", strtotime($date1new));
		}
		$date2new =$this->input->get("date2");
		 if($date2new != ""){
			 $date2 = date("Y-m-d", strtotime($date2new));
		 }
	  		
		//print_r($date1);
	  		$Employeename =$this->input->get("Employeename");
		$data['Employee'] = $this->Reading_model->master_fun_get_tbl_val1('shadminmaster', array('status'=>1 , 'type' => 2));
			...
			$query = $this->Reading_model->select_reading_list_web_count($date1,$date2,$Employeename); 
				 //print_r( $query); die();
				// $data["query"] = $this->master_model->search_data_reading($query);
                 $totalRows = $query['0']['total'];
				 $config = array();
                 $config["base_url"] = base_url() . "reading_master/reading_list/search?" . $_SERVER['QUERY_STRING'];
                 ...
                 $data["query"] = $this->Reading_model->select_reading_list_web($date1,$date2,$Employeename, $config["per_page"], $page); 
				 $data['count'] = $page + 1;
				 $data["logged_in"] = $_SESSION['logged_in'];
				
			 }else{
			
			$data["query"]=$this->Reading_model->shdailyreadingdetails_active_record();
            ...
            $data["query"] = $this->Reading_model->shdailyreadingdetails_active_record1($config["per_page"], $page);
		...
		$this->load->view('Admin/reading_view',$data);
		//$this->load->view('Admin/maxDate');
		$this->load->view('Admin/footer');
...
//		print_r("<pre>");print_r($data);
        if ($this->session->userdata('unsuccess') != null) {
         ...
         $data["query"] = $this->Reading_model->shdailyreadingdetails_active_record2($id);
         $data["details"] = $this->Reading_model->shdailyreadingdetails_reading_list_details($id);
...
//        $this->form_validation->set_rules('type', 'type', 'trim|required');
	...
	//print_r($data1); die();
        ...
        $this->load->view('Admin/header');
        $this->load->view('Admin/nav', $data);
        $this->load->view('Admin/reading_view_details', $data);
        $this->load->view('Admin/footer');
    }
...
//		print_r("<pre>");print_r($data);
        if ($this->session->userdata('unsuccess') != null) {
         ...
         $data["query"] = $this->Reading_model->shdailyreadingdetails_active_record2($id);
         $data["details2"] = $this->Reading_model->shdailyreadingdetails_reading_list_details();
         $data["details1"] = $this->Reading_model->shdailyreadingdetails_reading_list_details_id($id);
...
//echo "<pre>";	print_r($data1); 
			//die();
            $update = $this->Reading_model->master_fun_update1("shdailyReadingdetails", 'id', $this->uri->segment('3'), $data1);
			$cnt = 0;
			...
			//echo"<pre>";	print_r($id[$cnt]); print_r($row); 
				...
				$update = $this->Reading_model->master_fun_update1("shreadinghistory", 'id',$id_q, $data1_test);
				}else{
				...
				$this->Reading_model->master_fun_insert('shreadinghistory', $data1_test);
				}
				//print_r($data1_test);
				$cnt++;
			}
